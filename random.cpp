...
void testSort(void (*sortFunc)(long[], long), long arr[], long size, const string& sortName) {
    const int numTests = 50;  // Increase the number of tests for better averaging
    vector<long> memoryUsages;
    vector<double> durations;

    for (int i = 0; i < numTests; ++i) {
        srand(i); // Set the random seed using the current time
        gennum(arr, size);
        // Comment out the print statement for larger arrays
       // printarr(arr, size);
...
int main() {
    const long size = 500000; // Increase the array size for more meaningful tests
